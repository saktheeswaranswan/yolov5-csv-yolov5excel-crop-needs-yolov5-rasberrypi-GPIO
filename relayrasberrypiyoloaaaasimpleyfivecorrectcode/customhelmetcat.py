...
GPIO_PIN = 18  # GPIO pin number for the LED
GPIO.setmode(GPIO.BCM)
GPIO.setup(GPIO_PIN, GPIO.OUT)

# Flag to track cat presence
cat_present = False
...
while True:
    ...
    with open(csv_file, 'a', newline='') as f:
        ...
        for _, detection in detections.iterrows():
            ...
            if max_classes > 0:
                ...
                object_image_filename = f'{class_label}_{confidence:.2f}_{current_time}.jpg'
                object_image_path = os.path.join(folder_name, object_image_filename)
                cv2.imwrite(object_image_path, object_image)
                max_classes -= 1

                # Check if cat is missing, crop the person
                if class_label == 'person' and cat_present == False:
                    person_image_filename = f'person_{confidence:.2f}_{current_time}.jpg'
    ...
    if 'cat' in class_labels:
        cat_present = True
        GPIO.output(GPIO_PIN, GPIO.HIGH)  # Turn on the LED for cat class
    else:
        cat_present = False
